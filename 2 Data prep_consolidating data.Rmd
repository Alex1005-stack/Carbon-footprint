...
#B. 4 Calculate total Carbon footprint of Harvard per school and per student
  ...
  # Just look at 2017 data / Average_carbon_footprint_US of 15.8 according to monthly Energy Review November 2019 by the EPA (Table 1.7: Primary Energy Consumption, Energy Expenditures, and Carbon Dioxide Emissions Indicators) / Emissions per Capita column for 2017 / Reduction on the individual level per measure 
  ...
  CCF_per_student_red <- CCF_per_student %>%
    ...
    mutate(`Average annual carbon footprint in US per individual` = 15.8,
          ...
          select(-`2017`) %>%
    gather(key = "Type", value = "Carbon_footprint_in_MT", 2:8) %>%
    mutate(Action = ifelse(Carbon_footprint_in_MT > 0, "Carbon footprint", "Reduction measure"))
  
# Locking in order of types as to be displayed in the bar chart  
...
```
```{r}
# C. Reduce to 2017 top 20 companies and reduce to Organisation, Industry, Carbon_footprint_owned_MT
...
```{r}
# D. Replicate an average per year not selected by Harvard through randomly selecting 562 companies form the available sample for each year and comparing 1000 times and compare the average of these to the average we picked in our real data.
